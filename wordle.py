...
class WordInfo:
  def __init__(self, length, raw_words, common_words, guesses):
  ...
  def aggressiveGuess(self):
    ...
    def keyFxn(word):
      ...
      common = 0 if word in self.common else 1
      return (unknown, wrongPos, freq, common)
  ...
  def nonAgressiveGuess(self):
    ...
    commons = sortHelp([word for word in self.options if word in self.common])
    singles = sortHelp([word for word in self.options if len(set(word)) == len(word)])
    commonsingle = sortHelp([word for word in commons if word in singles])
    combined = commonsingle + commons + singles + sortHelp(self.options)
    return combined[0]
...
myword = WordInfo(word_length, raw_words, common_words, guesses)
